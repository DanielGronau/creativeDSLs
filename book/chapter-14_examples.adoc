...
Better-parse is the parser-combinator library we used in <<#parserLibrary,Chapter 9>>. It features a succinct hybrid DSL, which gives you a lot of flexibility. Here is one of the example parsers of the project:
...
Even without knowing the library, you can probably tell what conditions will be checked and what output you can expect. The DSL has a very consistent structure and descriptive naming conventions. If a condition is not met, you can provide your own using the `hint` infix function, but you don't have to, as there are already sensible default messages available. Even though Konform-kt doesn't have any features we haven't already covered, there is a lot to learn from how the DSL presents itself to the user and what makes it pleasant to use.

=== Conclusion

Exploring well-written DSLs can be a great source of inspiration. Frankly, a significant number of Kotlin DSLs currently available can be considered functional, but lack an engaging and enjoyable user experience because they rely heavily on a limited set of language features and miss the opportunity for a more exploratory approach. The examples presented in this chapter are intended to inspire you to strive for more, for DSLs that are creative and fun to use.
