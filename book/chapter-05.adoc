...
Even if your problem domain hasn't exactly an algebraic structure, some ideas in this chapter might be at least partially applicable. Take chemical equations as an example: Although though their components are not number-like, they contain concepts like "addition" of molecules and scalar multiplication by coefficients, and they can be modelled accordingly.
...
I hope the case study demonstrates that writing algebraic DLSs is usually not that difficult. There are still a few points to consider when deciding whether to use an algebraic DSL. For some use-cases it might be a stretch to use algebraic notation: Take e.g. sum and product types (a.k.a. `Either` and tuples like `Pair`and `Triple`), where using `+` and `*` would be surprising for some users, even though the underlying structure is algebraic. Even if the notation is appropriate for the use-case, there might be surprising behaviours like non-commutative multiplication (e.g. in case of quaternions and matrices) that could lead to usage errors.
