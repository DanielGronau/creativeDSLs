...
It is possible to have a "chained" version of the Loan Pattern, but it depends on the use case whether it looks better than the chained Builder Pattern. I would argue that in case of the SQL DSL from the last chapter the chained version wouldn't be a significant improvement. Here is how it could look like:
...
Here is a possible implementation of this DSL (sanity checks were omitted for brevity):
...
The Loan Pattern DSLs have several advantages over the classic Builder Pattern style, and is very common in Kotlin. It really shines when dealing with nested structures, but struggles a bit when the operations inside the block become a little more involved than simple assignments.
