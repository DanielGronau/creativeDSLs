...
The example code is written in a way where you generate the code manually via the `main()` method when the DSL has changed. This is a simple approach when you know that code changes don't happen very often. In the next section, we will discuss the use of an annotation processor instead.
...
....
  ...
  /-------------\        /-----------\            /--------------\
  | Annotations |<-------+ Processor +----------->|generated Code|
  \-------------/  uses  \-----+-----/ generates  \--------------/
        ^                      |                         ^
        |uses                  |inspects                 |uses
        |                      v                         |
...
class PhysicalQuantitesProcessorProvider : SymbolProcessorProvider {
...
[source,kotlin]
----
class GrpcEntityMapperProcessor(
