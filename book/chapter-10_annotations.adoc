...
Since it uses reflection, you need to include a dependency on the `kotlin-reflect` library, as described in <<#reflection,4.14 Reflection>>. Of course, the example could be improved in many ways, e.g. sometimes you need multiple source fields to calculate a target field.
