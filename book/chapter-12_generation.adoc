...
.Incremental Mode
====
KSP runs in https://kotlinlang.org/docs/ksp-incremental.html[incremental mode] by default, which means that it tries to avoid unnecessary re-processing of the sources. This is implemented by restricting the results of methods like `Resolver.getAllFiles()` and `Resolver.getSymbolsWithAnnotation()` to only the files that have changed.
