...
Writing a DSL for chemical equations is not an easy task because the notation is very concise and cannot be easily mimicked using Kotlin syntax. For our case study, we will try to recreate the string-based DSL from <<#chemical_equations_as_string,Chapter 9.2>> using a hybrid approach. We can use the same underlying model classes:
...
To capture values, you first define a value with the `capture<T>()` method. Then you can use it as a pattern on the left side of `then`, which always succeeds, but also stores the value. On the right side you can read the value from the predefined `val`. The first syntax example demonstrates the usage:
...
Despite the complexity of the topic, it wasn't too difficult to come up with a fairly usable hybrid DSL by using a combination of language features that complement each other.
