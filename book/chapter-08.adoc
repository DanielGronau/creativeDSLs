...
Chameleon classes and the phantom type implementation are conceptually similar, and it depends on the problem at hand whether a class with all the methods, or a data holder with extension methods is preferable. When the DSL needs to be called from Java, it should be considered that only the chameleon approach preserves the DSL syntax.

=== Using the Loan Pattern

So far, all examples followed a syntax close to the builder pattern. This doesn't have to be the case.
